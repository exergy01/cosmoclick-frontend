// pages/admin/types/index.ts - ОБНОВЛЕННАЯ ВЕРСИЯ с новыми полями
...
export interface AdminStats {
  ...
  // 🆕 НОВЫЕ ПОЛЯ - расширенная статистика обменов
  all_exchanges: {
    stars_to_cs: {
      total_exchanges: number;
      total_stars_exchanged: number;
    ...
    ccc_cs: {
      ccc_to_cs_exchanges: number;
      cs_to_ccc_exchanges: number;
      total_ccc_exchanged: number;
      total_cs_exchanged: number;
      exchanges_24h: number;
    };
    cs_ton: {
      cs_to_ton_exchanges: number;
      ton_to_cs_exchanges: number;
      total_cs_to_ton_amount: number;
      total_ton_to_cs_amount: number;
      ton_exchanges_24h: number;
    };
    totals: {
  ...
  // 🆕 НОВОЕ ПОЛЕ - статистика мини-игр
  minigames: {
    total_games: number;
  ...
  // 🆕 НОВОЕ ПОЛЕ - отладочная информация
  debug?: {
    activity_field_used: string;
    reason_values_found: number;
    top_reasons: string[];
