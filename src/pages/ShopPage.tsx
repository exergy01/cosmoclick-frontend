...
import { asteroidData, droneData, cargoData, getMaxItems, ShopItem } from '../data/shopData';
...
const ShopPage: React.FC = () => {
  ...
  const shopItems = {
    asteroids: asteroidData
      .filter((item) => item.system === currentSystem)
      .map((item) => {
        const isPurchased = player.asteroids.some((a) => a.id === item.id && a.system === item.system) || false;
        const isPreviousPurchased = item.id === 1 || player.asteroids.some((a) => a.id === item.id - 1 && a.system === item.system);
        return {
          ...item,
          isPurchased,
          isPreviousPurchased,
    ...
    drones: droneData
      .filter((item) => item.system === currentSystem)
      .map((item) => {
        const isPurchased = player.drones.some((d) => d.id === item.id && d.system === item.system) || false;
        const isPreviousPurchased = item.id === 1 || player.drones.some((d) => d.id === item.id - 1 && d.system === item.system);
        return {
          ...item,
          isPurchased,
          isPreviousPurchased,
  ...
  const buyItem = async (type: 'asteroid' | 'drones' | 'cargo', id: number, price: number) => {
    try {
      ...
      if (type === 'asteroid') {
        await buyAsteroid(id, price);
      }
      if (type === 'drones') {
        await buyDrone(id, price);
      }
      if (type === 'cargo') {
        const cargo = cargoData.find((item) => item.id === id && item.system === currentSystem);
        if (!cargo || !cargo.capacity) throw new Error('Invalid cargo capacity');
  ...
  const calculatePerHour = () => {
    ...
    const totalCccPerDay = player.drones.reduce((sum: number, d) => {
      const drone = droneData.find((item) => item.id === d.id && d.system === item.system);
      ...
      <div style={{ marginTop: '110px', paddingBottom: '130px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '15px', marginBottom: '10px' }}>
          {[
            ...
            { type: 'drones', count: `${player.drones.filter((d: { system: number }) => d.system === currentSystem).length}/${getMaxItems(currentSystem, 'drones')}` },
            { type: 'cargo', count: `${currentCargoLevel}/${getMaxItems(currentSystem, 'cargo')}` },
          ].map(({ type, count, amount }) => (
            <button
              key={type}
              onClick={() => setActiveTab(type === 'resources' ? 'asteroid' : (type as 'asteroid' | 'drones' | 'cargo'))}
              style={{
                flex: 1,
                padding: '8px 5px',
                background: 'rgba(0, 0, 0, 0.5)',
                border: activeTab === (type === 'resources' ? 'asteroid' : type) ? `4px solid ${colorStyle}` : `2px solid ${colorStyle}`,
                borderRadius: '15px',
                boxShadow: activeTab === (type === 'resources' ? 'asteroid' : type) ? `0 0 10px ${colorStyle}, inset 0 0 10px ${colorStyle}` : `0 0 10px ${colorStyle}`,
                color: '#fff',
                fontSize: '1rem',
                display: 'flex',
                flexDirection: 'column',
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', justifyContent: 'space-between', zIndex: 10 }}>
          {activeTab === 'asteroid' && (
            <>
              {shopItems.asteroids.map((item) => (
                <button
                  key={`asteroid-${item.id}`}
                  onClick={() => !item.isPurchased && item.isPreviousPurchased && buyItem('asteroid', item.id, item.price)}
                  disabled={item.isPurchased || !item.isPreviousPurchased}
                  style={{
                    flex: '1 1 calc(33.33% - 10px)',
                    padding: '10px',
                    background: item.isPurchased ? 'rgba(0, 255, 0, 0.5)' : item.isPreviousPurchased ? 'rgba(0, 240, 255, 0.3)' : 'rgba(119, 119, 119, 0.5)',
                    border: `2px solid ${colorStyle}`,
                    borderRadius: '10px',
                    boxShadow: `0 0 20px ${colorStyle}`,
                    color: item.isPurchased || !item.isPreviousPurchased ? '#fff' : '#000',
                    fontSize: '1rem',
                    display: 'flex',
                    flexDirection: 'column',
                    justifyContent: 'center',
                    alignItems: 'center',
                    aspectRatio: '1 / 1',
                    cursor: item.isPurchased || !item.isPreviousPurchased ? 'not-allowed' : 'pointer',
                    transition: 'transform 0.3s ease',
                    boxSizing: 'border-box',
                    height: 'auto',
                    visibility: 'visible',
                  ...
                  onMouseEnter={(e) => !item.isPurchased && item.isPreviousPurchased && (e.currentTarget.style.transform = 'scale(1.05)')}
                  onMouseLeave={(e) => (e.currentTarget.style.transform = 'scale(1)')}
                >
          ...
          {activeTab === 'drones' && (
            <>
              {shopItems.drones.map((item) => (
                <button
                  key={`drone-${item.id}`}
                  onClick={() => !item.isPurchased && item.isPreviousPurchased && buyItem('drones', item.id, item.price)}
                  disabled={item.isPurchased || !item.isPreviousPurchased}
                  style={{
                    flex: '1 1 calc(33.33% - 10px)',
                    padding: '10px',
          ...
          {activeTab === 'cargo' && (
            <>
              {shopItems.cargo.map((item) => (
                <button
                  key={`cargo-${item.id}`}
                  onClick={() => !item.isPurchased && item.isPreviousPurchased && item.id !== 5 && buyItem('cargo', item.id, item.price)}
                  disabled={item.isPurchased || !item.isPreviousPurchased || item.id === 5}
                  style={{
                    flex: '1 1 calc(33.33% - 10px)',
                    padding: '10px',
                    background: item.isPurchased ? 'rgba(0, 255, 0, 0.5)' : item.isPreviousPurchased ? 'rgba(0, 240, 255, 0.3)' : 'rgba(119, 119, 119, 0.5)',
                    ...
                    alignItems: 'center',
                    aspectRatio: '1 / 1',
                    cursor: item.isPurchased || !item.isPreviousPurchased || item.id === 5 ? 'not-allowed' : 'pointer',
                    transition: 'transform 0.3s ease',
                    boxSizing: 'border-box',
                    height: 'auto',
                    visibility: 'visible',
                  ...
                  onMouseEnter={(e) => !item.isPurchased && item.isPreviousPurchased && item.id !== 5 && (e.currentTarget.style.transform = 'scale(1.05)')}
                  onMouseLeave={(e) => (e.currentTarget.style.transform = 'scale(1)')}
                >
